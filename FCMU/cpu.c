...
Flag flags[] = {flagZ, flagN, flagH, flagC,flagNZ, flagNN, flagNH, flagNC,flagZ, flagN, flagH, flagC,flagNZ, flagNN, flagNH, flagNC};
...
functor parameterlessMap[] = {jp, call, ret, ei, di, halt, swap, cpl, bcd};
...
void decode(byte instruction) {
	...
	//flagged jumps and returns
	else if (instruction < 0xD8) {
		currentInstruction.func = flaggedFunctorMap[(instruction - 0xc0) / 8];
...
void execute() {
	...
	if (currentInstruction.func == add) addition();
	if (currentInstruction.func == inc) increment();
	if (currentInstruction.func == nopfunc) printf("nop\n");
}
